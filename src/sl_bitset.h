...
static const uint64_t kPerSeedCover = 1000;
...
public:
  ...
  void Alloc(uint64_t size) {
    ...
    uint64_t seed_cnt = slice_cnt / kPerSeedCover + 1;
    
    while (slice_cnt--) {
      slices_.insert(std::pair<uint64_t, uint64_t>(slice_cnt, 0));
    }
    while (seed_cnt--) {
      hash_seeds_.insert(std::pair<uint64_t, uint64_t>(seed_cnt, 0));
    }
  }
  ...
  void set(uint64_t pos) {
    ...
    uint64_t old_s = slices_[num];
    slices_[num] |= BitMask(SliceIndex(pos));
    refresh_seed(old_s, slices_[num], num);
...
private:
  ...
  std::map<uint64_t, uint64_t> hash_seeds_;
  uint64_t base_slice_;
  uint64_t base_hash_;
  std::size_t hash_;

  void refresh_seed(uint64_t old_s, uint64_t new_s, uint64_t num) {
    uint64_t index = num / kPerSeedCover;
    hash_seeds_[index] ^= old_s;
    hash_seeds_[index] ^= new_s;
  }
